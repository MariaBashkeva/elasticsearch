...
func NewElastic() (*Elastic, error) {
	...
	cfg := elasticsearch.Config{
		Addresses: []string{
			"http://localhost:9200", // Адрес вашего сервера Elasticsearch
		},
		Username: "maria", // Укажите имя пользователя
		Password: "hell123", // Укажите пароль
	}
...
func (el *Elastic) LoadData() {
	...
	defer res.Body.Close()
	err = ReadCsvFile("datasets/data.csv", es)
	if err != nil {
		log.Fatal(err)
	}
...
func (el *Elastic) GetPlaces(limit int, offset int) ([]Place, int, error) {
...
func (el *Elastic) GetClosestPlace(lat float64, lon float64) ([]Place, error) {
	query := fmt.Sprintf(`{
	"size": 3,
	"sort": [
    {
